...
async function loadSymbolAnalysis() {
  ...
  try {
    ...
    // 特定の話題（中東情勢など）に偏らないよう、取得件数を増やして幅広いニュースを渡す
    const generalNewsText = generalNews.slice(0, 25).map(n => `- ${n.headline}`).join('\n') || 'なし';

    const prompt = `あなたは個人投資家向けの情報整理アシスタントです。断定的な投資助言はせず、テクニカルと世界情勢の両面から材料を整理してください。
